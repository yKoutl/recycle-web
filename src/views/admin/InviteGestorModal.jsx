...
import { useInviteManagerMutation } from '../../store/auth/authApi';

const InviteGestorModal = ({ isOpen, onClose, themeColor }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLocalError('');
            await inviteManager(email).unwrap();
            setIsSent(true);
        } catch (error) {
                ...
                <div className="px-8 py-10 space-y-6">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="relative group">
                            <div className="absolute -left-12 top-1/2 -translate-y-1/2 -rotate-[30deg] text-orange-500/40">
                                <Leaf size={40} fill="currentColor" />
                            </div>
                            <div className="absolute -right-12 top-1/2 -translate-y-1/2 rotate-[30deg] text-orange-500/40">
                                <Leaf size={40} fill="currentColor" />
                            </div>
                            <div className="w-20 h-20 bg-orange-600 rounded-full flex items-center justify-center shadow-xl border-4 border-white/10 relative z-10">
                        ...
                        <div className="text-center space-y-6 animate-fade-in py-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-[10px] text-gray-400 leading-relaxed max-w-xs mx-auto font-bold tracking-wide">
                                    Invitación enviada a <span className="text-orange-500">{email}</span>.
                                </p>
                            </div>
                            <button
                                onClick={onClose}
                        ...
                        <div className="space-y-6">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div className="space-y-2 text-center">
                                    <label className="text-[10px] font-bold text-gray-500 tracking-wide">Correo electrónico</label>
                                    <div className="relative group">
                                        <input
                                            type="email"
                                            value={email}
                                            onChange={(e) => setEmail(e.target.value)}
                                            placeholder="gestor@institucion.com"
                                            required
                                            className="w-full text-center py-4 bg-white/5 border-2 border-white/10 rounded-2xl text-[13px] font-bold text-white outline-none focus:border-orange-500 transition-all placeholder:text-gray-600"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-4">
                                    <button
                                        type="submit"
                                        disabled={isLoading}
                                        className="w-full py-4.5 bg-orange-600 hover:bg-orange-500 text-white rounded-[20px] font-bold text-[12px] tracking-tight transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3"
                                    >
                                    ...
                                    <div className="flex items-center gap-3 text-white/20 justify-center">
                                        <ShieldCheck size={12} />
                                        <span className="text-[8px] font-black uppercase tracking-widest">Acceso único Nos Planet</span>
                                    </div>
                                </div>
                            </form>
                        </div>
...
export default InviteGestorModal;
