...
import { Plus, Edit2, Trash2, UserRoundCheck, Info, Search, Briefcase, Building2, UserCircle2, Handshake, Filter } from 'lucide-react';

import { useGetPartnersQuery, useDeletePartnerMutation } from '../../../store/partners';
            ...
            {filteredPartners.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-24">
                    {filteredPartners.map((partner) => (
                        <div
                            key={partner._id}
                            className="bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-xl hover:-translate-y-1 transition-all group relative overflow-hidden"
                        >
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                                <button
                                    onClick={() => handleEdit(partner)}
                                    className="p-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-xl hover:bg-[#018F64] hover:text-white transition-all shadow-sm"
                                    title="Editar"
                                >
                                    <Edit2 size={16} />
                                </button>
                                <button
                                    ...
                                    className="p-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm"
                                    title="Eliminar"
                                >
                                    <Trash2 size={16} />
                                </button>
                            ...
                            <div className="flex items-center gap-4 mb-6 relative z-10">
                                <div
                                    className="w-16 h-16 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg ring-4 ring-white dark:ring-gray-800 overflow-hidden shrink-0 transition-transform duration-500 group-hover:scale-110"
                                    style={{
